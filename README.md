...
새로 적용한 Seamless Co-op의 기본 세션 비밀번호는 `10301030`으로 자동 설정됩니다. 런처 1.3.3에는 ELDEN RING 1.17 + Seamless Co-op 1.9.9 전용 REN 호환 패치가 포함되어 있습니다. 공식 Seamless 설치가 감지된 경우에만 별도 REN DLL을 배치하고 실행 순서에 추가하며, 공식 `ersc.dll` 자체는 변경하거나 재배포하지 않습니다. 공식 Seamless가 더 새 버전으로 바뀌면 이 패치는 자동으로 로드 대상에서 제외됩니다.
